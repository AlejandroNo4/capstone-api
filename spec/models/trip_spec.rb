...
RSpec.describe Trip, type: :model do
  ...
  it 'validates length' do
    should validate_length_of(:destination).is_at_most(50)
  ...
  it 'validates with good examples' do
    is_expected.to allow_value('aval1destination').for(:destination)
  ...
  it 'validates with BAD EXAMPLES' do
    is_expected.to_not allow_value('').for(:destination)
    ...
    is_expected.to_not allow_value('invalidevenwithmorethan0characterswithwrongformatmorethan50').for(:destination)
  ...
  context 'table data base' do
    it { should have_db_column(:destination).of_type(:string) }
